...
struct state
{
    ...
    int *undo_count_list;
    bool ok;
    int side; // Handle to the left/right listview that is active
    HINSTANCE hInst;
};
...
void wm_notify(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    case ID_LISTVIEW_RIGHT:
        ...
        s->side = lpnmhdr->idFrom;
        switch (lpnmhdr->code)
        {
        case NM_RCLICK:
            /* store which listview is executing the operation in the the state*/
...
INT_PTR CALLBACK RangeProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    HWND hwnd_parent = GetParent(hDlg);
    struct state *s = (struct state *)GetWindowLongPtr(hwnd_parent, GWLP_USERDATA);
    ...
    case WM_INITDIALOG:
        ...
        /* position the dialog */
        HWND hwnd_listview = GetDlgItem(hwnd_parent, s->side);
        RECT r_lv;
        RECT r_dlg;
        GetWindowRect(hwnd_listview, &r_lv);
        GetWindowRect(hDlg, &r_dlg);
        MoveWindow(hDlg, r_lv.left, r_lv.top, r_dlg.right - r_dlg.left, r_dlg.bottom - r_dlg.top, TRUE);
    ...
    case WM_COMMAND:
        if (LOWORD(wParam) == IDOK)
        {
            ...
            condlist *list = s->side == ID_LISTVIEW_LEFT ? s->left : s->right;
            state_advance(s);
            condlist_add_range(list, from, to, by);  
            update(hwnd_parent);
...
void copy_to_clipboard(HWND hwnd)
{
    struct state *s = (struct state *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
    HWND hwnd_lv = GetDlgItem(hwnd, s->side);
    int count = ListView_GetSelectedCount(hwnd_lv);

    /* assume each number can only use 12 characters and set up a buffer to 
    hold all the info for the clipboard */
    int clipboard_buf_count = 12*count;
    wchar_t *clipboard_buf = (wchar_t *)HeapAlloc(GetProcessHeap(), 0,  clipboard_buf_count* sizeof(wchar_t));
    wchar_t *pos = clipboard_buf;

    /* iterate over all the selected items in the listview */
    int index = -1;
    for (int i = 0; i < count; i++)
    {
        index = ListView_GetNextItem(hwnd_lv, index, LVNI_SELECTED);
        ListView_GetItemText(hwnd_lv, index, 0, pos, 1024);
        while (*pos != L'\0')
        {
            pos++;
        }
    ...
    pos[-1] = L'\0';
    int len = pos-clipboard_buf;
    int ret;

    HGLOBAL hMem =  GlobalAlloc(GMEM_MOVEABLE, len*sizeof(wchar_t));
    memcpy(GlobalLock(hMem), clipboard_buf, len*sizeof(wchar_t));
    GlobalUnlock(hMem);
    OpenClipboard(hwnd);
    EmptyClipboard();
    ret = SetClipboardData(CF_UNICODETEXT, hMem);
    printf("%d\n", ret);

    CloseClipboard();

    HeapFree(GetProcessHeap(), 0, clipboard_buf);
}

void paste_from_clipboard(HWND hwnd) 
{
    struct state *s = (struct state *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
    HWND hwnd_lv = GetDlgItem(hwnd, s->side);
}
...
void wm_command(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    case ID_BUTTON_REDO:
        ...
        break;
    case ID_RANGEBOX:
        /* maybe use DialogBoxParam in the future to pass which listview is opening the dialog*/
...
__declspec(dllexport) int fepick_case(int *id, char **title, int n)
{
